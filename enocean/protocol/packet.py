...
class Packet(object):
    ...
    @staticmethod
    def create(packet_type, rorg, rorg_func, rorg_type, rorg_manufacturer=None,
               direction=None, command=None,
               destination=None,
               sender=None,
               learn=False, **kwargs):
        '''
        ...
        packet.rorg_manufacturer = rorg_manufacturer
    ...
    def build(self):
        ...
        ords = [0x55, (data_length >> 8) & 0xFF, data_length & 0xFF,
                len(self.optional), int(self.packet_type)]
...
class RadioPacket(Packet):
    ...
    @staticmethod
    def create(rorg, rorg_func, rorg_type, rorg_manufacturer=None,
               direction=None, command=None,
               destination=None, sender=None, learn=False, **kwargs):
        return Packet.create(PACKET.RADIO, rorg, rorg_func, rorg_type, rorg_manufacturer, direction, command, destination, sender, learn, **kwargs)
...
class UTETeachIn(RadioPacket):
    ...
    def send_response(self, response=TEACHIN_ACCEPTED):
        ...
        if not self.__communicator.teach_in:
            self.logger.info(
                'Communicator not set to teach-in mode, not sending UTE teach-in response.')
            return
